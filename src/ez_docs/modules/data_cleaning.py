...
def filter_data(location: str, constraint: str = "") -> list:
    try:
        ...
        if constraint: start_data.query(constraint[0], inplace=True)
        return [{col: str(start_data.iloc[line][col]) for col in start_data.columns} for line in range(len(start_data))]
